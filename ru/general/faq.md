...
### <a href="#how-to-delete-a-message" name="how-to-delete-a-message">5. Как удалить сообщение?</a>
...
Тем не менее, существует несколько обходных путей «спрятать» нежелательное сообщение:

 - У отправителя такого сообщения есть 120 секунд для его [редакции](/articles/ru/general/editing-messages), в частности всё сообщение можно заменить, например, дефисом (фактически удалив его).
 ...
 - Админы команды отправителя могут спрятать такое сообщение от не-админов (заметим, что соответствующая иконка обманчиво называется "Delete"). Понятно, что если отправитель не является админом к данной команде, то ему следует попросить кого-то из админов спрятать нежелательное сообщение от остальных не-админов.
 - Наконец, в виде крайней меры, админы команды отправителя могут удалить всю комнату, содержащую нежелательное сообщение. Полезно помнить, что даже в этом случае нежелательное сообщение может продолжать существовать в копиях базы данных или снимках экрана.  

Заметим, что отправленные в режиме «вне записи» сообщения в базу данных не попадают (но могут быть сохранены в снимках экрана).
