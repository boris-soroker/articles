...
Зелёные стрелки на снимках означают «кликните сюда». Светло-коричневые прямоугольные рамки на снимках обозначают участок снимка, на который следует обратить особое внимание.
 ...
 - [Как отредактировать опубликованное мною сообщение?](/articles/ru/faq/list#editing)
 - [Что такое «карточка сообщения»?](/articles/ru/faq/list#message-card)
 - [Где найти Kato-приложение для операционной системы Android?](/articles/ru/faq/list#android)
...
Заметим, что сообщения, состоящие из одних пробелов (или не содержащие символов вообще), не публикуются. 
...
Суть Markdown заключается в том, что разметка вносится в текст таким образом, чтобы не очень ухудшать его «читабельность» даже при отсутствии поддержки форматирования — в отличие от многих других языков, вроде HTML или XML.
...
В режиме Markdown в Kato можно публиковать красиво свёрстанные сообщения:
...
### <a href="#code-support" name="code-support">Как опубликовать фрагмент кода?</a>

Для того, чтобы опубликовать в виде сообщения фрагмент кода, который после копирования из истории сохраняет свою работоспособность, следует выбрать опцию "Code" в меню поля ввода:
...
В истории язык данного сообщение (при достаточном объёме кода) будет распознан автоматически: 

![clip.png](https://s3.amazonaws.com/kato-share/f3d5afe8950adee082c6cd1aa0943d6388a514eaf105b9fc771321419a0fde3/clip.png)

TODO: ссылка на длинные сообщения. 

### <a href="#otr" name="otr">Как послать «секретное» сообщение?</a>
...
Несколько важных уточнений:
 ...
 - Посланные в режиме «Без записи» сообщения **видны всем участникам**, на компьютерах которых открыта соответствующая комната.
 - Если после выключения режима «Без записи» участник команды, видящий посланные в режиме OTR сообщения, перезагрузит вкладку браузера Kato или закроет данную комнату, то посланные в режиме «Без записи» сообщения исчезнут из истории комнаты.
 - Режим «Без записи» не обеспечивает сколько-нибудь высокого уровня защиты (например, в любой момент может быть сделан снимок экрана) и не подходит для пересылки действительно важной секретной информации.
 - Загруженные на сервера Kato файлы хранятся отдельно от сообщений. Это означает, что загруженный в комнате с включённым режимом «Без записи» файл может оставаться доступным по его URL даже после того, как собственно ссылка на URL исчезнет из истории комнаты.

### <a href="#conferences" name="conferences">Как послать в сообщении ссылки на видео-конференции и аудио-конференции</a>

#### Внутренние видео-конференции, аудио-конференции и демонстрация экрана:

![clip.png](https://in.kato.im/789189aed7ef6ad82368ad5f88a4c83ab1e600cff445015764bdecb9c74be6f/clip.png)

#### Интеграция с Google Hangout:

![clip.png](https://in.kato.im/8de4c06d5a81967c5077e5683608ec8b7d93b66b36078752438e0789e9e17387/clip.png)
...
Символ "@", введённый в поле ввода некоторой комнаты, позволяет включить в сообщение **упоминание экранного имени участника** с доступом к этой комнате:

![clip.png](https://in.kato.im/e5d7af398325f38889ba5630b0e2ae3130427a500563e9cdae3011ac77f55/clip.png)

На снимке вверху видно меню, в котором перечислены экранные имена всех участников с доступом к данной комнате (исключая экранное имя самого пользователя).

Навигация по этому меню осуществляется клавишами стрелок вверх и вниз, а выбор делается клавишей "return":

![clip.png](https://in.kato.im/e0b588c14f2514ec5db8a1f1aad607a65bcde70dc840409d84d7baca823f88/clip.png)

После публикации этого сообщения упоминание выглядит в истории так:

![clip.png](https://in.kato.im/5cfbc57dc1b4c41f8cec501e4ab22af0eb41c9a2f07aebaaf124ba77b7a0efbf/clip.png)

### <a href="#room-mentions" name="room-mentions">Как послать в сообщении упоминание имени комнаты?</a>

Если мы в комнате X хотим привлечь внимание группы участников, про которую известно, что каждый член этой группы с высокой вероятностью установил уровень шума "Follow" для некоторой комнаты Y, то самый простой способ – **упоминание имени комнаты Y в комнате X**.

### <a href="#long" name="long">Как опубликовать очень длинный текст?</a>

Иногда возникает необходимость в публикации длинных сообщений, однако слишком объёмный текст может вызвать эффект «захламления» комнаты, заставляя пользователей долго прокручивать её историю в поисках следующих или предыдущих сообщений.

По этой причине сообщения длиной более 2000 символов помещаются в отдельные документы, при этом сообщение сокращается до ссылки на документ (плюс, иногда, начальный фрагмент текста сообщения), которая и публикуется в комнате. Эти отдельные документы могут быть открыты в отдельной вкладке браузера, для чего следует просто кликнуть на ссылку в истории. 

Функциональность «сокращения длинных сообщений» работает для сообщений, опубликованных в режимах "Markdown is off", "Markdown is on", "Code is on". Как и обычные сообщения, длинные тексты можно редактировать. 

#### Режим "Markdown is off"

Если опубликовать в комнате исходный текст длинного email-сообщения, то выглядеть это будет так:
...
#### Режим "Markdown is on"

Вставка большого текста в режиме "Markdown is on" выглядит так:
...
Активировав режим "Code is on", пользователь может создать сообщение, состоящее из программного кода: 

![clip.png](https://s3.amazonaws.com/kato-share/84564f4b182bbd783a3162f2aa4e202339c7162c44e3ea2d1f1f68f676f4f0ff/clip.png)

В истории комнаты показывается ссылка:
...
**История** комнаты располагается между заголовком комнаты и полем ввода комнаты и состоит из множества всех сообщений, опубликованных в данной комнате в хронологической последовательности (самые старые сообщения располагаются вверху, самые недавние - внизу):
...
Следует обратить внимание на **разделители** групп соообщений, содержащие время публикации последнего сообщения в группе:

![clip.png](https://in.kato.im/cf40c70fb87b1cbb1fde68fd98256ffa86aac8c382e747258e4e6f6d9d872bd7/clip.png)

«Свежие» (то есть появившиеся после последнего открытия данной комнаты) сообщения в моменты, когда вкладка Kato была не в фокусе, маркируются жёлтой вертикальной полосой слева:

![clip.png](https://in.kato.im/58828fdfbac1a0d01d47467831e3c1ef899822c7bd09a105d5b8eeab59116ef7/clip.png)

Ещё один чрезвычайно важный (хотя и малозаметный) элемент истории - вертикальная полоса, напоминающая шпалы, слева от каждого сообщения в истории, которая видна в ситуации, когда курсор мышки расположен в пределах этого сообщения в истории:

![clip.png](https://in.kato.im/7a9801e88a270163b7a14bc83996fd9f8398c1935f8c5578b4b5083de30f71/clip.png)

Клик на «шпалы» открывает **карточку сообщения**, подробно описанную [здесь]().
...
### <a href="#editing" name="editing">Как отредактировать опубликованное мною сообщение?</a>

Пользователь может изменить текст последнего отправленного им сообщения в каждой определённой комнате, и для завершения этой операции, называемой также «редактированием сообщения», пользователь располагает 120 секундами с момента публикации этого сообщения. 

В меню поля ввода имеется опция редактирования:

![clip.png](https://in.kato.im/80da21b970d5987b909bb65f9cedeeddd6b28eaaab3e5bb87d480c7840e4336/clip.png)

Более простой способ начать редактирование последнего (опубликованного не ранее 120 секунд назад) сообщения данного пользователя - кликнуть на поле ввода данной комнаты и нажать на клавишу «стрелка вверх». 

Подходящее по требованиям сообщение (если такое есть) будет скопировано в поле ввода данной комнаты. Редактируемое сообщение в истории будет помечено зеленой полосой слева, справа будет добавлена иконка карандаша, а поле ввода будет выделено зеленой рамкой:

![clip.png](https://in.kato.im/2c06f181a283fae4b266afbfb88912def1f2003254c03f272d10d71bd4318db2/clip.png)

Пользователю следует изменить текст и нажать клавишу "return". После этого новый текст заместит собой первоначальное сообщение:

![clip.png](https://in.kato.im/47c58ee6f4ff1e8c135098086fc7ec8376d2077d98fce7b0c4f80b699ca640c3/clip.png)

Обратите внимание на иконку «карандаш» у отредактированного сообщения, которая видна всем участникам, у которых открыта история данной комнаты – это сигнал о том, что данное сообщение было отредактировано.
...
Некоторые полезные детали:
 ...
 - С помощью комбинации "ctrl+z" в Windows ("command+z" на Mac OS X) можно восстановить текст отменённых (например, клавишей "esc") изменений.
 - Если пользователь нажимает клавишу "return" после истечения лимита времени на редактирование, то изменённый текст будет опубликован в качестве нового сообщения.
 - Информация о факте редактирования текста сообщения в базе данных не сохраняется – старое сообщение просто заменяется в базе данных на новое. В частности, это означает, что иконка «карандаш» исчезнет из интерфейса любого участника после закрытия этим участником данной комнаты и открытия её заново.
...
### <a href="#message-card" name="message-card">Что такое «карточка сообщения»?</a>
...
### <a href="#android" name="android">Где найти Kato-приложение для операционной системы Android?</a>

Ссылка на Google Play – <a href="https://play.google.com/store/apps/details?id=im.kato" target="_blank">https://play.google.com/store/apps/details?id=im.kato</a>.

![clip.png](https://in.kato.im/7058d8a46a3e1d2cb98c31411f62d41e7b28fc16e3d23cad721dc12935236c0b/clip.png)

### <a href="#ios" name="ios">Где найти Kato-приложение для операционной системы iOS?</a>

Ссылка на Apple App Store – <a href="https://itunes.apple.com/us/app/kato/id862791364" target="_blank">https://itunes.apple.com/us/app/kato/id862791364</a>.

![clip.png](https://in.kato.im/b7383ca91bc7f8798bea1bd8223077c2f71e4e476c4b4ead4e36c11d7cffb146/clip.png)

### <a href="#osx" name="osx">Где найти Kato-приложение для операционной OS X?</a>

Ссылка на файл .pkg – [http://labs.kato.im/Kato.pkg](http://labs.kato.im/Kato.pkg).
...
### <a href="#windows" name="windows">Как создать ярлык для Kato-приложения в операционной системе Windows?</a>

Ниже приведена последовательность действий для создания ярлыка Kato-приложения (Chrome shortcut) в операционной системе Windows:

(1) Откройте в браузере Chrome страницу [https://app.kato.im](https://app.kato.im).

(2) Выберите меню настроек в тулбаре браузера Chrome.

(3) Выберите секцию «Инструменты» ("Tools").

(4) Выберите пункт «Создать ярлык приложения» ("Create application shortcuts").
...
Например, по директиве "GH79", использованной в любой комнате команды разработчиков Kato, Katobot выводит заголовок описания записи с номером 79 в соответствующем GitHub репозитории:
...
### <a href="#pill" name="pill">Что такое «пилюля»?</a>

Уведомлений, которые мы называем «пилюлями» за их форму, являются основными сигналами для пользователя Kato о наличии и числе непрочитанных сообщений в комнатах, командах и окнах:

![clip.png](https://in.kato.im/3473dab63da0493a492e242a16b89e8927322a9789bcc15da190b72bf1c4bdb6/clip.png)

На картинке выше показан пример красной пилюли справа от кнопки "Fast-forward". 

### <a href="#pill-color" name="pill-color">Что означают разные цвета пилюль?</a>

![clip.png](https://in.kato.im/8d29fc85435d25f739c47695630d0bd486e15cd4afab6d5a759d9a952debd73/clip.png)

Как видно из картинки наверху, пилюли бывают трёх цветов – красного, чёрного, и серого.

Цвета пилюль означают разные уровни важности сообщения:

- Красный – высший уровень важности. Он означает, что среди непрочитанных сообщений имеется:
 - любое сообщение в комнате типа 1:1, либо
 - сообщение в любой комнате, содержащее прямой оклик данного пользователя, либо
 - сообщение в любой комнате, содержащее оклик комнаты, для которой данным пользователем установлен уровень шума "Follow".
- Чёрный – средний уровень важности. Он означает, что среди непрочитанных сообщений нет сообщений «красного» уровня, но имеется сообщение в любой комнате, для которой данным пользователем установлен уровень шума "Follow".
- Серый – низший уровень важности. Он означает, что среди непрочитанных сообщений нет сообщений «чёрного» уровня, но имеется сообщение в любой комнате, для которой данным пользователем установлен уровень шума "Watch".

### <a href="#pill-locations" name="pill-locations">Где в пользовательском интерфейсе могут встретиться пилюли?</a>
...
**«Всплывающее уведомление»** – это небольшое окно с сообщением (или с его фрагментом), появляющееся на дисплее (обычно в правом верхнем или в правом нижнем углу, в зависимости от операционной системы):

![clip.png](https://in.kato.im/3207fa53814cb70c646b61218b8ccdcb59c8cf0d6a54b06bc0cb00af767a1b4c/clip.png)

Всплывающее уведомление исчезает в зависимости от операционной системы, браузера и настройки.
